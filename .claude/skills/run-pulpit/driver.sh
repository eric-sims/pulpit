#!/bin/bash
# Build, launch and drive Pulpit on the iOS simulator.
#
...
DD="${DD:-$ARTIFACTS/dd}"
BUNDLE_ID="com.ericsims.pulpit"
APP="$DD/Build/Products/Debug-iphonesimulator/Pulpit.app"
...
cmd_build() {
  ...
  xcodebuild -project "$PROJECT_DIR/Pulpit.xcodeproj" \
    -scheme Pulpit \
    -destination "id=$udid" \
    -derivedDataPath "$DD" \
    -quiet build
